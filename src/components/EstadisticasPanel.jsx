...
// Paleta de colores Premium y Coherente para Gr├íficos
...
// Obtener tema visual refinado seg├║n el themeMode de la aplicaci├│n
...
const StatCard = ({ icon, label, value, sub, accent, theme }) => {
  return (
    <div className="hover:scale-[1.03] hover:shadow-lg transition-all duration-300 cursor-pointer" style={{ borderRadius: 20, transform: 'translateZ(0)', willChange: 'transform' }}>
      <div style={{
        background: theme.isLight ? `linear-gradient(135deg, ${accent}12, ${accent}06)` : `linear-gradient(135deg, ${accent}18, ${accent}06)`,
...
export default function EstadisticasPanel({ tickets, usuarios, themeMode = 'dark', user }) {
  ...
  // Tickets por d├¡a (para vista de un mes espec├¡fico)
  ...
  // T├®cnico responsable (quien m├ís tickets resolvi├│ en total)
  ...
  // T├®cnico con m├ís tickets asignados (filtrado por per├¡odo)
  ...
  return (
    <div style={{ width: '100%', overflowY: 'auto', maxHeight: 'calc(100vh - 180px)' }} className="custom-scrollbar px-3 pb-6 printable-area">
      ...
      <div className="print-hidden">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 24 }}>
          <div>
            <h1 style={{ color: visualTheme.titleColor, fontSize: 24, fontWeight: 900, display: 'flex', alignItems: 'center', gap: 10, letterSpacing: '-0.03em' }}>
              ...
              Estad├¡sticas de Ticketera
            </h1>
            <p style={{ color: visualTheme.subColor, fontSize: 12, marginTop: 2, fontWeight: 550 }}>Informaci├│n consolidada de productividad y dependencias</p>
          </div>
          <div className="flex items-center gap-3">
            <select
              value={anioSeleccionado}
              onChange={e => setAnioSeleccionado(e.target.value)}
              className="px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 cursor-pointer outline-none border bg-white/95 border-slate-200 text-slate-800 focus:ring-2 focus:ring-indigo-500/20 dark:bg-slate-900/80 dark:border-neutral-800 dark:text-neutral-100 dark:focus:ring-indigo-500/30"
            >
            ...
            <button onClick={() => window.print()} className="px-4 py-2 rounded-xl text-sm font-semibold bg-gradient-to-r from-[#065E94] to-[#043d63] dark:from-cyan-500 dark:to-cyan-600 hover:opacity-90 text-white shadow-[0_4px_15px_-3px_rgba(6,94,148,0.4)] dark:shadow-[0_4px_15px_-3px_rgba(6,182,212,0.4)] transition-all flex items-center gap-2 hover:-translate-y-0.5">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6 p-2">
          <StatCard theme={visualTheme} icon="tickets" label="Total Tickets" value={ticketsFiltrados.length} sub="Per├¡odo seleccionado" accent={visualTheme.accentNuevos} />
          ...
          <StatCard theme={visualTheme} icon="urgent" label="Urgentes" value={urgentes} sub="Atenci├│n inmediata" accent={visualTheme.accentUrgentes} />
        ...
        <div className={`grid grid-cols-1 ${!isJefe ? 'lg:grid-cols-2' : ''} gap-4 mt-4`}>
          <SectionCard title="Pedidos por Dependencia (Top)" icon={ICONS.building} theme={visualTheme}>
            {porDependencia.length === 0 ? (
              <p style={{ color: visualTheme.subColor, textAlign: 'center', padding: '32px 0', fontSize: 13, fontWeight: 500 }}>Sin informaci├│n de dependencias</p>
            ) : (
          ...
          {!isJefe && (
            <SectionCard title="Productividad (M├ís Resueltos Totales)" icon={ICONS.star} theme={visualTheme}>
        ...
        {!isJefe && (
          <div style={{ marginTop: 16 }}>
            <SectionCard title="Tickets Asignados por T├®cnico (Carga de Trabajo)" icon={ICONS.users} theme={visualTheme}>
      ...
      {/* ========================================================= */}
      {/* VISTA IMPRESI├ôN ÔÇö DISE├æO MODERNO PREMIUM v2               */}
      {/* ========================================================= */}
      <div className="hidden print:block w-full" style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px', padding: '0 4px' }}>
          {[
            { label: 'Total Solicitudes', value: ticketsFiltrados.length, color: '#065E94', sub: 'en el per├¡odo' },
            ...
            { label: 'Urgentes', value: urgentes, color: '#ef4444', sub: 'atenci├│n prioritaria' },
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px' }}>
            ...
            <p style={{ color: '#1e293b', fontSize: '13px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '1px' }}>
              {mesSeleccionado === 'todos'
                ? `Evoluci├│n Mensual de Incidentes ÔÇö ${anioSeleccionado}`
                : `Distribuci├│n Diaria ÔÇö ${MESES[parseInt(mesSeleccionado)]} ${anioSeleccionado}`
              }
            </p>
          </div>
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            {mesSeleccionado === 'todos' ? (
              ...
              // Vista mensual: BarChart por d├¡a
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ color: '#0369a1', fontSize: '11px', fontWeight: 600 }}>
              {enProceso} tickets a├║n en proceso
            </p>
            {urgentes > 0 && (
              <p style={{ color: '#dc2626', fontSize: '11px', fontWeight: 700, marginTop: '4px' }}>
                ÔÜá {urgentes} ticket{urgentes > 1 ? 's' : ''} de car├ícter urgente
              </p>
            )}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div>
            <p style={{ color: '#94a3b8', fontSize: '10px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '1px' }}>
              Sistema de Control de Tickets Inform├íticos ÔÇö CTI
            </p>
            <p style={{ color: '#cbd5e1', fontSize: '10px', marginTop: '2px' }}>
              Documento generado autom├íticamente. Confidencial de uso interno.
            </p>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <p style={{ color: '#475569', fontSize: '12px', fontWeight: 700, letterSpacing: '0.3px' }}>Dir. de Inform├ítica</p>
          </div>
        </div>
      </div>

      {/* ========================================================= */}
      {/* BACKUP ÔÇö DISE├æO ANTERIOR v1 (COMENTADO)                   */}
      ...
      <div className="hidden print:block w-full text-black bg-white" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
        ...
        <div className="flex items-center justify-between mb-8 pb-4" style={{ borderBottom: '3px solid #00a2bb' }}>
          ...
          <div style={{ textAlign: 'right', borderLeft: '2px solid #00a2bb', paddingLeft: '16px' }}>
            <p style={{ fontSize: '14px', fontWeight: 800, textTransform: 'uppercase', marginBottom: '4px', fontFamily: 'Arial, sans-serif', color: '#065E94' }}>Direcci├│n de Inform├ítica Delegaci├│n III</p>
            <p style={{ fontSize: '12px', fontWeight: 600, color: '#555' }}>Sistema CTI - Reporte Oficial</p>
          </div>
        </div>
        ... (dise├▒o completo v1 preservado aqu├¡) ...
      </div>
      */}
    </div>
  );
